...
**Linux distributions** refers to _operating systems_ include Linux kernel and supporting system software and libraries.
 ...
 - Popular Linux distributions include Debian, Fedora, Ubuntu, Red Hat, SUSE, and etc.
...
If you want to know the details of `usr` directory, you can use the long format version of `ls` by adding `-l` to `ls`:
...
- Arguments can be categorized in **positional arguments** and **keyword arguments (options)**.
