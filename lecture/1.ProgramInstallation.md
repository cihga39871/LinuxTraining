...
4. Some software can run without install.
...
Usually, we need to de-compress the source files.

The table shows common file type and the decompression methods.

| File Extension        | Decompression Method             |
| --------------------- | -------------------------------- |
...
First, go to the directory, then run the configure script.
...
The current value of path return you to the [command prompt](https://www.computerhope.com/jargon/c/commprom.htm), like:
...
The output file is `sample_fastqc.html` and `sample_fastqc.zip`. You can view the quality control report in the html file.
